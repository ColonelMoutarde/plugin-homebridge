...
class homebridge extends eqLogic
{
    ...
    public static function Pluginsuported() {
        return ['openzwave', 'rfxcom', 'edisio', 'mpower', 'mySensors', 'Zibasedom', 'virtual', 'camera', 'weather', 'philipsHue', 'enocean', 'wifipower', 'alarm', 'mode', 'apcupsd', 'btsniffer', 'dsc', 'rflink', 'mysensors', 'relaynet', 'remora', 'unipi', 'eibd', 'thermostat', 'netatmoThermostat', 'espeasy', 'jeelink', 'teleinfo', 'tahoma', 'protexiom', 'lifx', 'wattlet', 'rfplayer', 'openenocean'];
    }
    ...
    public static function PluginWidget() {
        return ['alarm', 'camera', 'thermostat', 'netatmoThermostat', 'weather', 'mode'];
    }
    ...
    public static function PluginMultiInEqLogic() {
        return ['LIGHT_STATE', 'ENERGY_STATE', 'FLAP_STATE', 'HEATING_STATE', 'SIREN_STATE', 'LOCK_STATE'];
    }
    ...
    public static function DisallowedPIN() {
        return ['000-00-000', '111-11-111', '222-22-222', '333-33-333', '444-44-444', '555-55-555', '666-66-666', '777-77-777', '888-88-888', '999-99-999', '123-45-678', '876-54-321'];
    }
    ...
    public static function PluginToSend() {
        ...
        $pluginCompatible = self::Pluginsuported();
        $pluginWidget     = self::PluginWidget();
        foreach ($plugins as $plugin) {
            $plugId = $plugin->getId();
            if ($plugId == 'homebridge') {
                continue;
            } elseif (in_array($plugId, $pluginWidget)) {
                array_push($PluginToSend, $plugId);
            } elseif (in_array($plugId, $pluginCompatible) && !in_array($plugId, $pluginWidget) && config::byKey('sendToApp', $plugId, 1) == 1) {
                array_push($PluginToSend, $plugId);
            } elseif (!in_array($plugId, $pluginCompatible) && config::byKey('sendToApp', $plugId, 0) == 1) {
    ...
    public static function dependancy_info() {
        $return = ['log'           => 'homebridge_update',
            'progress_file' => jeedom::getTmpFolder('homebridge') . '/dependance'
            ];
    ...
    public static function dependancy_install($fromRepair = false) {
        ...
        $returnArray = ['script' => dirname(__FILE__) . '/../../resources/install_homebridge.sh ' . network::getNetworkAccess('internal', 'ip'),
    ...
    public static function deamon_start($_debug = false) {
        if (log::getLogLevel('homebridge') == 100) {
            $_debug = true;
        }
    ...
    public static function repairHomebridge($reinstall = true) {
        ...
        if ($reinstall) {
            ...
            exec('npm uninstall homebridge-jeedom --save');
            log::add('homebridge', 'info', 'suppression homebridge');
            exec('npm uninstall homebridge --save');
        }
     ...
     * @return type
     */
    public static function discovery_cmd(array $plugin = []) {
        $return           = [];
        $genericisvisible = [];
     ...
     * @return array
     */
    public static function discovery_multi(array $cmds) {
        $array_final = [];
     ...
     * 
     * @param array $cmds
     * @param array $eqLogics
     * @return string
     */
    public static function change_cmdAndeqLogic(array $cmds, array $eqLogics) {
        ...
        if ($nbr_cmd != 0) {
            ...
            while ($i < $nbr_cmd) {
                ...
                array_push($eqLogic_array, [$eqLogic_id, $new_eqLogic_id, $eqLogic_name]);
                $i++;
            }
        ...
        $new_cmds    = ['cmds' => $cmds];
        $new_eqLogic = ['eqLogics' => $eqLogics];
        $news        = [$new_cmds, $new_eqLogic];
        return $news;
    }
    ...
    /**
     * 
     * @param array $objectsATraiter
     * @param array $eqlogicsATraiter
     * @return array
     */
    public static function delete_object_eqlogic_null(array $objectsATraiter, array $eqlogicsATraiter) {
        $retour = [];
